...
-   `Miranda Jordan` do not have any direct connections between any organization/location.

-   `The Intern` is a bit suspicious in talking to `Mrs. Money` and `The Lookout` with over 24 and 25 messages respectively, `The Intern` also talks to `Boss` as well as an organization namely `V. Miesel Shipping`

-   `Sam` seems to be entity that has the most influence among the organizations.

-   `V. Miesel Shipping` seems to be the entity that has the most influence among the persons.
:::
...
-   `The Intern` was talking to the `Boss`, `The Middleman`, `The Lookout`, `Mrs Money` and `Mako` in week 1 whereas `The Intern` was talking to `Mrs Money`, `The Lookout` and `Sam` in week 2
:::
