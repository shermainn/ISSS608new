...
pacman::p_load(tidyverse, jsonlite,
               tidygraph, ggraph,
               SmartEDA, dplyr, visNetwork, igraph, knitr, DT, stringr, cronologia, tidytext, dplyr)
```
...
mc3_nodes_final <- mc3_nodes_cleaned %>%   slice(used_node_indices) %>%   mutate(new_index = row_number())
```
...
```{r}
ggraph(mc3_graph,         layout = "fr") +   geom_edge_link(alpha = 0.3,                   colour = "gray") +   geom_node_point(aes(color = `type`),                    size = 4) +   geom_node_text(aes(label = type),                   repel = TRUE,                   size = 2.5) +   theme_void()
```
...
::: panel-tabset
## 💡 Observations from Communication network map
...
A table is generated to see the distinct people and organization and groups as well as vessels whose radio communication Clepper has intercepted.
...
-   ::: panel-tabset
    ...
    # Find the max number of rows across all subtypes
    max_len <- max(nrow(persons), nrow(organizations), nrow(groups), nrow(vessels))
    ...
    table_combined <- tibble(
      ...
      Vessel = c(vessels$Vessel, rep(NA, max_len - nrow(vessels)))
    )
    ...
    Next, we’ll combine all the people, groups, organizations and vessels to check what are the people/organization/groups/vessels each individual talk about or talk to:
    ...
    # redefine persons with name and new_index
    persons <- all_nodes %>%
      filter(sub_type == "Person") %>%
    ...
    -   Next, we combine every exchanges Sam receives or sends and check for the contents of the exchanges. We can see which organization or group or individual he talks about.
    ...
    ### 8th Individual: Rodriguez

    ```{r}

    ```

    ### 9th Individual: Sailor Shift
