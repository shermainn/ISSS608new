...
gg1 <- ggplot(globalPop, 
  ...
  geom_point(aes(size = Population,
             ...
             alpha = 0.7, 
             show.legend = FALSE) +
              #legend still appears
  scale_colour_manual(values = country_colors) +
  scale_size(range = c(2, 12)) +
  labs(x = '% Aged', 
       y = '% Young')

gg2 <- ggplot(globalPop, 
       aes(x = Old, 
           y = Young, 
           size = Population, 
           colour = Country)) +
...
g1_plotly <- ggplotly(gg1)
g2_plotly <- ggplotly(gg2)

subplot(g1_plotly,g2_plotly, nrows = 1, shareX = TRUE, shareY = TRUE) %>% layout(title = "With and Without Legend")
