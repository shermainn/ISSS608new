...
I will be plotting a dot plot to compare the results between the female and male students who scored well (above 80) in English.

```{r}
high_english <- exam_data[exam_data$ENGLISH > 80, ]
ggplot(high_english, aes(x = GENDER, y = ENGLISH)) +
  geom_dotplot(
    binaxis = 'y', 
    stackdir = 'center', 
    dotsize = 0.7, 
    binwidth = 1,
    fill = "dodgerblue") +
  labs(title = "Students Scoring Above 80 in English by Gender",
       x = "Gender",
       y = "English Score") +
  theme_minimal()
```
